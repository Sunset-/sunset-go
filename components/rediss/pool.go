...
func NewRedisPool(db int, host string, password string, defaultExpiration time.Duration) *RedisCli {
	pool := &redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			ops := make([]redis.DialOption, 0)
			ops = append(ops, redis.DialDatabase(db))
			if len(password) > 0 {
				ops = append(ops, redis.DialPassword(password))
			}
	...
	return &RedisCli{pool: pool, defaultExpiration: defaultExpiration}
}

func (c RedisCli) Close() (err error) {
	if c.pool != nil {
		return c.pool.Close()
	}
	return nil
